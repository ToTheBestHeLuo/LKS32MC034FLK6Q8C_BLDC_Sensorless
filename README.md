...
## 2025.1.15
### 更细内容
sG协议初步完善。现在Master（飞控端）可以通过sG对Slave（ESC电调端）进行参数的实时调整，此外，可将调整好的参数固化到Slave的Flash中。sG支持双向通信，且有编程以及油门两种工作模式。通过发送特定的命令字使其工作在油门传输模式时，Slave端实时发送电气频率除以4的数据给Master，因此，Master可以以最高10KHz左右的频率实现转速闭环控制，最后，当Master发送过多的零油门数据给Slave，则Slave会自动退出油门状态，重新进入编程状态。
## 2025.2.13
### 异常描述
电机存在较大的起动失败几率。
### 解决措施
修改Polling状态下的换相检测函数，并于OpenLoop环节将换相检测函数的检测结果投入运行，如果长时间没有检测到换相，则进行强制换相，这种做法能极大程度减少起动失败的概率。
